"""
As model32, but with a fixed small scale in p(x|z).
The p(x|z) location still varies with |z|...
"""
import os
...
class Decoder(tf.keras.Model):
    ...
    def call(self, z, **kwargs):
        ...
        pxz = DiscretizedLogistic(mu, tf.math.log(0.01), low=0.0, high=1.0, levels=256)
        return pxz


class Model39(Model, tf.keras.Model):
    def __init__(self):
...
if __name__ == "__main__":
    # PYTHONPATH=. CUDA_VISIBLE_DEVICES=1 nohup python -u models/model39.py > models/model39.log &
    ...
    for i in [0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0]:
        ...
        print(np.std(pxz.loc), np.mean(pxz.loc))
