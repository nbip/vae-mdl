...
class Decoder(tf.keras.Model):
    def __init__(self, **kwargs):
        ...
        self.dense = layers.Dense(2 * 2 * 256, activation=tf.nn.tanh)
        ...
        self.convt = tf.keras.Sequential(
            [
                *[
                ...
                layers.Conv2D(
                    2 * 3, kernel_size=1, strides=1, padding="same", activation=None
                ),
            ]
        )
...
class Model08(Model, tf.keras.Model):
    def __init__(self):
        super(Model08, self).__init__()
    ...
    def decode(self, z):
        ...
        pxz = tfd.Normal(loc + 0.5, tf.exp(tf.nn.tanh(logscale)))
    ...
    def save(self, fp):
        self.save_weights(f"{fp}_08")

    def load(self, fp):
        self.load_weights(f"{fp}_08")
