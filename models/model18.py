"""
As model18.py, but with mdl loss
Not too good samples or reconstructions
"""
import os
...
class Decoder(tf.keras.Model):
    ...
    def call(self, z, **kwargs):
        ...
        logits = self.deconvs(h)
        logits = tf.reshape(logits, [*z.shape[:-1], 32, 32, self.n_mix * 10])
        pxz = MixtureDiscretizedLogistic(logits)
        return pxz


class Model18(Model, tf.keras.Model):
    def __init__(self):
    ...
    def save(self, fp):
        self.save_weights(f"{fp}_18")

    def load(self, fp):
        self.load_weights(f"{fp}_18")
...
if __name__ == "__main__":
    # PYTHONPATH=. CUDA_VISIBLE_DEVICES=1 nohup python -u models/model18.py > models/model18.log &
