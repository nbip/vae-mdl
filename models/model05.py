"""
Try out the mixture of discretized logistics loss
"""
import os
...
from utils import GlobalStep, MixtureDiscretizedLogistic, fill_canvas, setup_data
...
class Decoder(tf.keras.Model):
    ...
    def call(self, z, **kwargs):
        ...
        out = self.deconvs(h)
        out = tf.reshape(
            out, [*z.shape[:-1], self.out_shape[0], self.out_shape[1], self.n_mix * 10]
        )
...
class Model05(Model, tf.keras.Model):
    ...
    def _plot_samples(self, x):
        ...
        samples = np.clip(pxz.sample()[0], 0.0, 1.0)
...
if __name__ == "__main__":
    # PYTHONPATH=. CUDA_VISIBLE_DEVICES=1 nohup python -u models/model05.py > models/model05.log &
    ...
    plt.imshow(samples)
    plt.axis("off")
    plt.savefig(f"./assets/model05_samples")

    plt.clf()
    plt.imshow(recs)
    plt.axis("off")
    plt.savefig(f"./assets/model05_recs")

    plt.clf()
    plt.imshow(imgs)
    plt.axis("off")
    plt.savefig(f"./assets/model05_imgs")
