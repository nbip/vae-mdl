...
by scaling the scale with the magnitude of the latent variable input.
...
class Decoder(tf.keras.Model):
    ...
    def call(self, z, **kwargs):
        ...
        # scale the scale with the input magnitude
        magnitude = tf.reduce_mean(tf.math.abs(z), axis=-1)
        logstd = logstd - tf.math.log(magnitude[:, :, None, None, None])
        mu = mu / magnitude[:, :, None, None, None]
...
class Model37(Model, tf.keras.Model):
    def __init__(self):
...
if __name__ == "__main__":
    # PYTHONPATH=. CUDA_VISIBLE_DEVICES=1 nohup python -u models/model37.py > models/model37.log &
