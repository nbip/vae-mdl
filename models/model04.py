"""
As model03.py, bit with larger conv architecture.
"""
import os
...
if __name__ == "__main__":
    # PYTHONPATH=. CUDA_VISIBLE_DEVICES=1 nohup python -u models/model04.py > models/model04.log &
    ...
    plt.imshow(samples)
    plt.axis("off")
    plt.savefig(f"./assets/model04_samples")

    plt.clf()
    plt.imshow(recs)
    plt.axis("off")
    plt.savefig(f"./assets/model04_recs")

    plt.clf()
    plt.imshow(imgs)
    plt.axis("off")
    plt.savefig(f"./assets/model04_imgs")
