"""
Discretized logistic loss. Try to use batchnorm the q(z|x) location
parameter to force it be similar to the prior in the aggregate.
This causes a race where the encoder parameters try to outrun the momentum of the batchnorm
to get the location parameter into a smaller space than p(z).
"""
import os
...
class Encoder(tf.keras.Model):
    def __init__(self, n_latent):
        ...
        self.bn = layers.BatchNormalization(momentum=0.1, axis=-1)
    ...
    def call(self, x, training=True, **kwargs):
        ...
        # mu = mu - tf.reduce_mean(mu, axis=-1, keepdims=True) / tf.math.reduce_std(mu, axis=-1, keepdims=True)
...
class Model34(Model, tf.keras.Model):
    def __init__(self):
...
if __name__ == "__main__":
    # PYTHONPATH=. CUDA_VISIBLE_DEVICES=1 nohup python -u models/model34.py > models/model34.log &
    ...
    model.load("best")
    qzx = model.encode(x)
    print(np.std(qzx.loc))

    r = tf.random.uniform([10, 128, 20])
    bn = layers.BatchNormalization(axis=-1, momentum=0.1)

    for i in range(10):
        c = bn(r, training=True)
        print(np.mean(c))
        c = bn(r, training=False)
        print(np.mean(c))
