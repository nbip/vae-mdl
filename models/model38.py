"""
As model32, but trying to make the decoder input-magnitude invariant
by scaling nn(z) by |z|.
Doesn't cut it yet, I think the output needs to depend also on |z|.
"""
import os
...
class Decoder(tf.keras.Model):
    def __init__(self, n_latent):
        ...
        self.fc_res = layers.Dense(n_latent, activation=tf.nn.relu)
        self.fc_std1 = layers.Dense(n_latent, activation=tf.nn.relu)
        self.fc_std2 = layers.Dense(np.prod(output_shape), activation=None)

        self.fc_res2 = layers.Dense(n_latent, activation=tf.nn.relu)
    ...
    def call(self, z, **kwargs):
        ...
        # ---- Try to diminish the magnitude effect from the prior on the logscale
        h = self.fc_res(z) - tf.math.abs(z)
        h = self.fc_std1(h)
        h = self.fc_std2(h)
        logstd = tf.reshape(h, [*z.shape[:-1], 32, 32, 3])

        h = self.fc_res2(z) - tf.math.abs(z)
        h = self.fc(h)
        ...
        mu, _ = tf.split(out, num_or_size_splits=2, axis=-1)
...
class Model38(Model, tf.keras.Model):
    def __init__(self):
...
if __name__ == "__main__":
    # PYTHONPATH=. CUDA_VISIBLE_DEVICES=1 nohup python -u models/model38.py > models/model38.log &
    ...
    for i in [0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0]:
        ...
        pxz = model.decode(zn)
        print(f" |z|: {i}, pxz.scale: {np.mean(np.exp(pxz.logscale)):.4f}")
        print(
            f" |z|: {i}, std(pxz.loc): {np.std(pxz.loc):.2f}, mean(pxz.loc): {np.mean(pxz.loc):.2f}"
        )
